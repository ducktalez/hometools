...
SVG_STAR = '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.5l2.95 6.02 6.65.97-4.8 4.68 1.13 6.63L12 17.68 6.07 20.8l1.13-6.63-4.8-4.68 6.65-.97L12 2.5z"/></svg>'
SVG_STAR_EMPTY = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2.5l2.95 6.02 6.65.97-4.8 4.68 1.13 6.63L12 17.68 6.07 20.8l1.13-6.63-4.8-4.68 6.65-.97L12 2.5z"/></svg>'
...
SVG_BOARD = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/><polyline points="9,14 11,16 15,11"/></svg>'
SVG_EDIT = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 17.25V21h3.75L18.81 8.94l-3.75-3.75L3 17.25z"/><path d="M13.13 6.19l3.75 3.75"/></svg>'
...
# Lightning-bolt overlay used as smart-playlist marker in the corner of
# the playlist folder card and as a standalone icon.  Filled glyph so it
# reads at small sizes (10–14 px) over the playlist logo.
SVG_SMART_PLAYLIST = '<svg viewBox="0 0 24 24" fill="currentColor"><polygon points="13,2 4,14 11,14 9,22 20,9 13,9"/></svg>'
...
# Combined "Filtern" popover trigger (Bewertung + Favorit + Genre) — see
# docs/IMPLEMENTATION_PLAN.md "UI-Template-Vereinheitlichung" Phase 2.
SVG_FILTER = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="4,4 20,4 14,12.5 14,19 10,21 10,12.5"/></svg>'
...
# Double-chevron icons for the BPM-adjust popup ("Langsamer"/"Schneller" —
# halve/double a mis-detected value by one octave, see
# player_js/_track_render.py::_openBpmAdjustMenu).
SVG_CHEVRONS_DOWN = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="7,7 12,12 17,7"/><polyline points="7,13 12,18 17,13"/></svg>'
SVG_CHEVRONS_UP = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="7,17 12,12 17,17"/><polyline points="7,11 12,6 17,11"/></svg>'
...
SVG_DOTS = '<svg viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg>'
...
# Chromecast / AirPlay button — HTML5 Remote Playback API trigger.
# Classic "TV with wifi-arc lower-left" glyph.
SVG_CAST = (
...
# Skip-Intro button — "skip forward to next marker" glyph (Netflix-style).
SVG_SKIP_INTRO = (
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" '
    'stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'
