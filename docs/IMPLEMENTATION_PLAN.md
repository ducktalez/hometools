...
### Playlist-Redesign — Sprint 4: Cross-Device Playlist Sync
- **Playlist-Changelog** — Neues JSONL-File `<cache_dir>/playlists/changelog.jsonl` für Playlist-Mutationen (create/delete/rename/add/remove/reorder). Eigenes Log, kein Audit-Log.
- **Cross-Device-Sync** — Playlists über alle verbundenen Clients synchronisieren. Ansatz: Polling-basiert, Client ruft periodisch `GET /api/<media>/playlists` auf und merged mit lokalem State. Konfliktlösung: „Last write wins" mit Timestamp.
