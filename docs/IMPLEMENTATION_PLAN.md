...
`recentMoveTargets.ts`, `catalogCache.ts`, `offlineDownloads.ts`). Never
bulk-port via `eval()` — tried once, broke prod (see git log 2026-08-06).
`legacy.ts` sits unused in tree as raw material — don't re-enable as-is.
