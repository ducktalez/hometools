...
### Playlist-Redesign — Sprint 3: Filesystem-Ordner Reorder — Server-seitig (optional)
- **custom_order-Persistierung pro Ordner (Server-seitig)** — Aktuell wird die benutzerdefinierte Reihenfolge in `localStorage` (client-seitig) gespeichert. Optionale Erweiterung: Server-seitige Persistierung über `<cache_dir>/custom_order/<hash>.json` für Cross-Device-Sync. Neues Backend-Modul.
