...
### Playlist-Redesign — Offene Diskussionspunkte
- **DISKUSSION:** „Neu hinzugefügte Titel oben einordnen" — Aktuell werden neue Items am Ende eingefügt. Alternative: am Anfang (index 0) einfügen. Könnte als `HOMETOOLS_PLAYLIST_INSERT_POSITION=top|bottom` umgesetzt werden. Vorläufig: am Ende (konsistent mit Spotify).

### Playlist-Redesign — Sprint 5: Erweiterte Sync-Strategien
- **Optimistic UI** — Mutationen sofort lokal anwenden, bei Server-Fehler rollback (offline-first).
- **Changelog-Retention** — `changelog_<server>.jsonl` rotieren (max 1000 Zeilen) oder bei `make clean` löschen.
- **Konfigurierbares Polling-Intervall** — `HOMETOOLS_PLAYLIST_SYNC_INTERVAL` Env-Var (Standard 30s).
