...
- **Ausgeblendete Songs: Grund-Anzeige entfernt** — Das kleine Badge neben dem Titel ausgeblendeter (niedrig bewerteter) Songs, das den Grund als Text zeigte ("Bewertung: X★"), wurde entfernt (`hiddenBadge`/`.hidden-badge` in `_search_filter.py`/`_table_view.py`). Das eigentliche Ausblenden/Grauen (`_hiddenShown`, `.track-item--hidden-shown`, der „Ausgeblendet"-Filter-Chip) bleibt unverändert bestehen — nur die Text-Begründung ist weg, da nicht relevant für den Nutzer.
