...
11. **Automatische Favoriten-Playlist** — Virtuelle Playlist-Karte „Favoriten" (`__favorites__`) wird auf der Root-Startseite vor den User-Playlists angezeigt, wenn mindestens ein Favorit existiert. Kein separater Playlist-Eintrag auf dem Server — wird client-seitig aus `_savedFavorites` und `allItems` erzeugt. Klick öffnet Browse-Ansicht (`showUserPlaylistView`), Play-Button spielt ab (`playUserPlaylist`). **DnD-Reorder:** `_currentPlaylistId = '__favorites__'` aktiviert DnD. Reihenfolge wird client-seitig in `localStorage` (`ht-favorites-order`) als JSON-Array von `relative_path`-Strings gespeichert. `_sortFavoritesByOrder()` wendet die gespeicherte Reihenfolge beim Öffnen an; neue Favoriten ohne gespeicherte Position landen am Ende. `reorderPlaylistItem()` erkennt `__favorites__` und führt die Verschiebung lokal durch (kein Server-Call).
