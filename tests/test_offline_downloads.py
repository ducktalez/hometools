...
class TestDownloadUI:
    ...
    def test_download_button_in_waveform(self):
        ...
        from hometools.streaming.core.server_utils import render_base_css, render_player_js
...
class TestServiceWorkerOfflineSupport:
    ...
    def test_service_worker_handles_offline_streams(self):
        ...
        assert 'FETCH_FROM_INDEXEDDB' in sw
        assert 'Offline — stream not cached' in sw
...
class TestOfflinePlayback:
    ...
    def test_js_handles_online_offline_events(self):
        ...
        assert 'window.addEventListener(\'online\'' in js or \
               'addEventListener(\'online\'' in js
        assert 'window.addEventListener(\'offline\'' in js or \
               'addEventListener(\'offline\'' in js
